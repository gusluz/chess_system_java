...
public class ChessMatch {
	...
	private List<Piece> piecesOnTheBoardList = new ArrayList<Piece>();
	private List<Piece> capturedPieceList = new ArrayList<Piece>();
	...
	public Color getCurrentPlayColor() {
	...
	public ChessPiece[][] getPieces() {
		ChessPiece[][] matChessPieces = new ChessPiece[board.getRows()][board.getColumns()];
		for (int i = 0; i < board.getRows(); i++) {
			for (int j = 0; j < board.getColumns(); j++) {
				matChessPieces[i][j] = (ChessPiece) board.piece(i, j);
			}
		}
	...
	public ChessPiece performChessMove(ChessPosition sourcePosition, ChessPosition targetPosition) {
		...
		check = (testCheck(opponent(currentPlayerColor))) ? true : false;
		
		if(testeCheckMate(opponent(currentPlayerColor))) {
			checkMate = true;
		}
		else {
			nextTurn();
	...
	private ChessPiece king(Color color) {
		List<Piece> pieceList = piecesOnTheBoardList.stream().filter(x -> ((ChessPiece) x).getColor() == color).collect(Collectors.toList());
